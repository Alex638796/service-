...
async def _notify_sleeping(
    ...
    """Helper to update the UI to the Sleeping status during a FloodWait."""
    text = _build_progress_text(
        cfg, start_id, end_id, current_id,
        forwarded, skipped, failed, "sleeping", sleeping_seconds=wait_s
    )
